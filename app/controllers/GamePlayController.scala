...
class GamePlayController extends Controller {
  ...
  var socketMap =
    Map.empty[(Long, Long), (Enumerator[JsValue], Channel[JsValue])]
  ...
  def hasGameStarted(game_id: Long) = Action {
    inTransaction {
      val selectGameStatus = from(gameStatusTable)(g =>
        where(g.game_id === game_id) select (g)).single
  ...
  def dealCards(game_id: Long, round_number: Int) = Action {
    ...
    inTransaction {
      val playerStatusList = from(playerStatusTable)(ps =>
        where(ps.game_id === game_id and ps.status <> "Out") select (ps))
      val currentRound = from(gameStatusTable)(g =>
        where(g.game_id === game_id) select (g)).single
      val gameHand = from(gameHandTable)(g =>
        where(g.game_id === game_id and g.round_number === currentRound.status) select (g))
      ...
      if (cardsNotDealt) {
        ...
        for (p <- playerStatusList.toList) {
          ...
          Logger.info(
            "inserting hand for player" + p.player_id + " hand= " + playerCardList(
              p.player_id))
          gameHandTable.insert(
            new GameHand(game_id,
                         currentRound.status,
                         p.player_id,
                         cards.mkString(",")))
        }
  ...
  def getCards(game_id: Long) = Action(parse.json) { req =>
    ...
    inTransaction {
      val game = from(gameStatusTable)(g =>
        where(g.game_id === game_id) select (g)).single
      val gameHand = from(gameHandTable)(gp =>
        where(
          gp.game_id === game_id and gp.round_number === game.status and gp.player_id === player.player_id) select (gp))
  ...
  def getAllHands(game_id: Long) = Action { req =>
    ...
    inTransaction {
      val game = from(gameStatusTable)(g =>
        where(g.game_id === game_id) select (g)).single
      val allHands = from(gameHandTable)(gp =>
        where(gp.game_id === game_id and gp.round_number === game.status) select (gp))
  ...
  def getFinalStandings(game_id: Long) = Action { req =>
    inTransaction {
      ...
      val winnerPlayer = from(playerStatusTable)(ps =>
        where(ps.game_id === game_id and ps.status <> "Out") select (ps)).single
      update(playerStatusTable)(
        p =>
          where(
            p.player_id === winnerPlayer.player_id and p.game_id === game_id) set (p.position := 1))
      val playerStatusList = from(playerStatusTable)(ps =>
        where(ps.game_id === game_id) select (ps) orderBy (ps.position))
      Ok(Json.toJson(playerStatusList.toList))
    }
  }

  def takeAction(game_id: Long, player_id: Long) = WebSocket.using[JsValue] {
    request =>
      Logger.info(
        s"Creating socket for Game_Id = ${game_id}, Player_Id = ${player_id}")

      val (out, channel) = socketMap
        .get((game_id, player_id))
        .getOrElse(Concurrent.broadcast[JsValue])
      ...
      val in = Iteratee.foreach[JsValue] { msg =>
        {
          ...
          action match {
            case JsDefined(JsString("GameStatus")) =>
              ...
              inTransaction {
                val playerStatusList = from(playerStatusTable)(ps =>
                  where(ps.game_id === game_id and ps.status <> "Out") select (ps) orderBy (ps.position))
                val channels = socketMap.filter(p =>
                  (p._1._1 == game_id && p._1._2 == player.player_id))
                channels.foreach(f =>
                  f._2._2 push (Json.toJson(playerStatusList.toList)))
              }
            ...
            case JsDefined(JsString("Challenge")) =>
              ...
              inTransaction {
                val playerHandList = from(gameHandTable)(gh =>
                  where(
                    gh.game_id === game_id and gh.round_number === roundResult.round_number) select (gh))
                val channels = socketMap.filter(p => (p._1._1 == game_id))
                channels.foreach(f => f._2._2 push (Json.toJson(roundResult)))
                channels.foreach(f =>
                  f._2._2 push (Json.toJson(playerHandList.toList)))
            ...
            case JsDefined(JsString("RoundResult")) =>
              ...
              Logger.info(roundResult toString)
              Logger.info(
                s"Closing all sockets after round ${roundResult.round_number} for gameId ${game_id}");
              inTransaction {
                ...
                update(roundResultTable)(r =>
                  where(
                    r.game_id === roundResult.game_id and r.round_number === roundResult.round_number)
                ...
                if (roundResult.result.equals("WON")) {
                  update(playerStatusTable)(p =>
                    where(
                      p.player_id === roundResult.player_bet_id and p.game_id === game_id) set (p.num_of_cards := p.num_of_cards.~ + 1))
                  val lostPlayer = from(playerStatusTable)(p =>
                    where(
                      p.player_id === roundResult.player_bet_id and p.game_id === game_id) select (p)).single
                  if (lostPlayer.num_of_cards > 5) {
                    //TODO : Find out why order of below two statements matter!
                    // Updating Final Standing
                    update(playerStatusTable)(p =>
                      where(
                        p.player_id === roundResult.player_bet_id and p.game_id === game_id) set (p.position := playerStatusList.toList.size))

                    update(playerStatusTable)(p =>
                      where(
                        p.player_id === roundResult.player_bet_id and p.game_id === game_id) set (p.status := "Out"))
                  ...
                  update(playerStatusTable)(p =>
                    where(
                      p.player_id === roundResult.player_challenge_id and p.game_id === game_id) set (p.num_of_cards := p.num_of_cards.~ + 1))
                  val lostPlayer = from(playerStatusTable)(p =>
                    where(
                      p.player_id === roundResult.player_challenge_id and p.game_id === game_id) select (p)).single
                  if (lostPlayer.num_of_cards > 5) {
                    // Updating Final Standing
                    update(playerStatusTable)(p =>
                      where(
                        p.player_id === roundResult.player_challenge_id and p.game_id === game_id) set (p.position := playerStatusList.toList.size))

                    update(playerStatusTable)(p =>
                      where(
                        p.player_id === roundResult.player_challenge_id and p.game_id === game_id) set (p.status := "Out"))
