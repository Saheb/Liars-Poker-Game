...
class JoinGameController extends Controller {
  ...
  def joinGame(game_id: Long) = Action(parse.json) { request =>
    ...
    inTransaction {
      val selectQuery = from(playerStatusTable)(ps =>
        where(ps.player_id === player.player_id and ps.game_id === game_id) select (ps))
      if (selectQuery.isEmpty) {
        val lastPlayer = from(playerStatusTable)(ps =>
          where(ps.game_id === game_id) select (ps) orderBy (ps.position desc))
        if (lastPlayer.isEmpty)
          playerStatusTable.insert(
            new PlayerStatus(player.player_id,
                             player.name,
                             game_id,
                             1,
                             2,
        ...
        val selectPlayerStatus =
          PlayerStatus.getPlayerStatusById(player.player_id, game_id)
